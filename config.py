import os, re


BOT_TOKEN = os.environ.get("BOT_TOKEN", "6141105459:AAG2QgssXPV2at540w1CextAZJIrSRH8Kq4")
API_ID = int(os.environ.get("API_ID", "3566507"))
API_HASH = os.environ.get("API_HASH", "194e4ba8b6787e11274f47ae63eb56ba")
OWNER = int(os.environ.get("OWNER", "1654009706"))
BOT_USERNAME = os.environ.get('BOT_USERNAME', "RenameSfbot")

FORCE_SUBS = os.environ.get("FORCE_SUBS", "VipMoviez")
LOG_CHANNEL = int(os.environ.get("LOG_CHANNEL", "-1001720888155"))

DB_URL = os.environ.get("DB_URL", "mongodb+srv://Rolexfilter:Rolexfilter@rolex.0o0saa0.mongodb.net/?retryWrites=true&w=majority")
DB_NAME = os.environ.get("DB_NAME", "renamebot-premium")

STRING = os.environ.get("STRING", "")
BOT_PIC = os.environ.get("BOT_PIC", "https://graph.org/file/cca849a2f63053fa3f622.jpg")







# if you need to add verify system then dm me on telegram

SHORTNER_URL = os.environ.get("SHORTNER_URL", "")
SHORTNER_API = os.environ.get("SHORTNER_API", "")
TOKEN_TIMEOUT = os.environ.get("TOKEN_TIMEOUT", "")
